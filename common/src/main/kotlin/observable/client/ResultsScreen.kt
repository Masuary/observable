...
import observable.shadow.imgui.*
...
class ResultsScreen : Screen(TranslatableComponent("screens.observable.results")) {
    companion object {
        ...
        private val minecraftBehaviorsField by lazy {
            Class.forName("observable.shadow.imgui.ImguiKt").getField("MINECRAFT_BEHAVIORS")
        }
        private val cStrMethod by lazy {
            Class.forName("observable.shadow.imgui.HelpersKt").getMethod("getCStr", ByteArray::class.java)
        }

        private fun enableMinecraftBehaviors() {
            minecraftBehaviorsField.setBoolean(null, true)
        }

        private fun cStr(bytes: ByteArray): String {
            return cStrMethod.invoke(null, bytes) as String
        }

        fun initCtx(reinit: Boolean = false) {
            if (inited && !reinit) return

            enableMinecraftBehaviors()
    ...
    fun applyMapFilter() {
        val filter = cStr(filterBuf).lowercase()
        filterMap = entryMap.map { (key, list) ->
            key to list.filter {
                filter in it.type.lowercase()
